...
import { G, Use, Rect } from 'react-native-svg';
...
class AG extends Component {
  ...
  render() {
    return <G {...this.props} opacity={this.props.opacity} />;
  }
}
...
function calcPosition(node) {
  ...
  node.children.forEach((item) => {
    item.navigation.y =
      -(totalHeight - node.shape.height) / 2 + node.point.y + offsetY;
...
function makeAnimate(node) {
  ...
  node.children.forEach((item) => {
    animateMap[item.data.node_id] = {
      opacity: new Animated.Value(0),
      y: new Animated.Value(node.point.y),
    };
  });
}
...
class Navigation extends Component {
  ...
  dh() {
    this.state.activeNode.children.forEach((item) => {
      ...
      Animated.parallel([
        Animated.timing(animateMap[item.data.node_id].opacity, {
          toValue: 1,
          duration: 300,
  ...
  render() {
    ...
    let childrenList = this.state.activeNode.children.map((node, index) => {
      return (
        ...
        >
          <Rect width={node.shape.width} height={node.shape.height} />
          <Use href={'#' + node.data.node_id} />
        </DH.G>
      );
    });

    return <G>{childrenList}</G>;
  }
}

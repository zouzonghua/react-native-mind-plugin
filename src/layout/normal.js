...
const normal = {
  init: function () {
    ...
    this.allNode.forEach((node, index) => {
      ...
      node.point.y =
        -(node.parent.area.height - node.parent.shape.height) / 2 +
        (node.area.height - node.shape.height) / 2 +
        node.parent.point.y +
        node.parent.point.childOffsetY;

      //记录偏移
      if (node.isLeaf()) {
        node.parent.point.childOffsetY +=
          node.shape.height + nodeStyle.blankBottom;
      } else {
        node.parent.point.childOffsetY += node.area.height;
      }
      node.point.x =
        node.parent.point.offsetX +
        node.parent.shape.width +
        nodeStyle.blankLeft;
